...
- (void)zj_viewWillAppearForIndex:(NSInteger)index{
    if (self.oldIndexPath) {
        ...
        if (![model.title isEqualToString:HgSongInfo.shared.title]) {
            model.isPlay = NO;
            [self.mainTableView reloadRowsAtIndexPaths:@[self.oldIndexPath] withRowAnimation:UITableViewRowAnimationNone];
        }
    }
}
...
-(void)changeIndex:(NSDictionary<NSKeyValueChangeKey,id> *)change{
    ...
    [self.mainTableView reloadRowsAtIndexPaths:@[indexPath,self.oldIndexPath] withRowAnimation:UITableViewRowAnimationNone];
...
- (void)viewDidLoad {
    ...
    [self createTableViewRefresh:false];
    // 防止 block  循环 引用
